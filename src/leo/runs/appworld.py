...
APPWORLD_RUN_PROMPT_SUPPLEMENT = (
    ...
    "\nAppWorld is a closed simulated environment. Do not use real external services, real accounts, or third-party Python packages."
    ...
    "\nIf list_appworld_apis returns a task_strategy_hint, follow that strategy directly unless a tool result proves it wrong."
    "\nIf task_strategy_hint includes example_code, prefer adapting and executing that complete code path over manually probing one item at a time."
    "\nIf list_appworld_apis returns recommended_next_tool, call that tool next unless that same tool has already failed for the same app."
    "\nWhen execute_appworld_task_strategy is available for the relevant app, prefer it over manually chaining many execute_appworld_code calls."
    "\nIf any AppWorld tool result includes recommended_next_tool or strategy_guidance, call that tool on the very next turn instead of continuing manual probing."
    "\nDo not manually retype, copy, or reconstruct access tokens from printed tool output. Keep auth and follow-up API calls inside one code snippet or switch to execute_appworld_task_strategy."
    "\nIf login succeeds and a recommended strategy exists, stop manual exploration and call execute_appworld_task_strategy immediately."
    ...
    "\nWhen inspecting values with execute_appworld_code, use print(...) so the observed value is returned to you."
